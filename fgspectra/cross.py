...
class DecorrelatedFactorizedCrossSpectrum(Model):
    ...
    .. math:: xC_{\ell}^{(ij)} = \Delta_{\rm decor} f_{decor}(\nu_i, \nu_j) f(\nu_j) f(\nu_i) C_{\ell}
    ...
    .. math::  f_{\rm decor} = I(3) + \Delta_{\rm decor} \begin{pmatrix}
        0 & r_{12} & 1 \\
        r_{12} & 0 & r_{23} \\
        1 & r_{23} & 0
        \end{pmatrix}
    ...
    def eval(self, decor=None, f_decor=None, sed_kwargs={}, cl_kwargs={}):
        ...
        cl_args : list
            Arguments for which the `cl` is evaluated.
        decor : float
            Decorrelation factor, by which f_decor is rescaled
        f_decor : ndarray
        ...
        cross : ndarray
            Cross-spectrum. The shape is ``(..., freq, freq, ell)``.
        """
        if decor == 1.: #Assumes not decorrelation
            decorrelation = np.ones(f_decor.shape)
        else:
...
class LogDecorrelatedFactorizedCrossSpectrum(Model):
    ...
    def eval(self, decor=None, nu_0=None, nu_0_prime=None, sed_kwargs={}, cl_kwargs={}):
        """Compute the model at frequency and ell combinations.
        ...
        decor : float
            Decorrelation factor, by which f_decor is rescaled
        nu_0, nu_0_prime: float
            Reference freqeuncies. By convention, nu_0_prime is the largest of the two.
        Returns
        -------
        cross : ndarray
            Cross-spectrum. The shape is ``(..., freq, freq, ell)``.
        ...
        for i, nu_i in enumerate(nu_array):
            for j, nu_j in enumerate(nu_array):
                f_decor[i,j] = np.log(min(nu_i, nu_j)/nu_j)**2 / np.log(nu_0/nu_0_prime)**2 
